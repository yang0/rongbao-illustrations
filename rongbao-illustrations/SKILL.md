...
默认视觉 IP 是“绒宝”：以 `assets/rongbao.png` 为身份参考，保留黄色圆形主体、两片青绿色叶耳、棕黑大眼、橙色腮红和橙色手脚；在本 Skill 中转译成简洁的黑色手绘线稿与克制平涂色块。绒宝认真做一件荒诞但成立的事，必须参与画面的核心动作，不能只是站在旁边当装饰。
...
仅组合模式使用目标设计 Skill 的 QA、输出路径和交付契约，不套用本 Skill 的白底、16:9、`assets/<article-slug>-illustrations/` 或 `qa-checklist.md` 默认值。交付前额外读取 `references/rongbao-identity.md`，确认黄色圆形主体、青绿色叶耳、大棕黑眼睛、橙色腮红、橙色手脚等身份锚点仍可辨识，并确认角色与场景共享媒介、笔触、边缘、颗粒、环境光和投影；如用户明确改变锚点，按用户要求记录该偏差。若出现参考图材质残留、抠图感或贴片感，只做一次针对角色媒介融合的定向迭代，保持构图、标题、色彩和身份锚点不变。
