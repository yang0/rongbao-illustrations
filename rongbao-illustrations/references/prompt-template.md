...
以下模板只适用于本 Skill 的正文配图原生模式。组合到其他设计 Skill 时，不要套用本模板的白底手绘默认值；改为透传目标 Skill 契约，并附带 `assets/rongbao.png` 作为角色参考图。组合透传时补充：角色材质、笔触、边缘、颗粒、环境光和投影必须适配目标媒介，与场景共享媒介和光线；若仍有贴片感，只做一次角色媒介融合迭代，保持构图、标题、色彩和身份锚点不变。
...
Color use:
Black for main line art and structure. Use restrained yellow, teal, and orange accents to keep 绒宝 recognizable. Orange for main flow/path/arrows. Red only for key warnings/problems/results. Blue only for secondary notes or feedback/system state.
