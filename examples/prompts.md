...
没有写角色名时默认使用绒宝；“这个IP / 该IP”也不会切换默认角色。英文别名不区分大小写。
...
明确写出未注册的 IP 名称时，Skill 会返回支持列表（绒宝、牙仔），不会猜测替代角色。
...
当请求同时包含角色/IP信号和目标画幅时，`$rongbao-illustrations` 只负责意图路由与已选角色身份参考，目标设计 Skill 负责画幅、构图、材质、光线、文字和输出。组合请求会保留 `create|prompt` 语义，并将注册表中每个选中角色的参考图传递；角色要与场景共享媒介和光线，绒宝身份色默认鲜亮，禁止综合色偏/统一降饱和/复古做旧；贴片感只做一次角色融合迭代；正文配图仍走本 Skill 原生白底手绘模式。
